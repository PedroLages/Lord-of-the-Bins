...
export type ConflictType =
  | 'skill-mismatch'        // Operator lacks skill for assigned task
  | 'availability'          // Operator unavailable on assigned day
  | 'double-assignment'     // Operator assigned to multiple tasks same day
  | 'understaffed';         // Task has fewer operators than required

// Severity level for conflicts
export type ConflictSeverity = 'critical' | 'warning';

// Individual schedule conflict
export interface ScheduleConflict {
  id: string;
  type: ConflictType;
  severity: ConflictSeverity;
  day: WeekDay;
  operatorId?: string;      // Involved operator (if applicable)
  operatorName?: string;    // For display
  taskId?: string;          // Involved task (if applicable)
  taskName?: string;        // For display
  message: string;          // Human-readable description
  // Additional context based on conflict type
  requiredSkill?: string;   // For skill-mismatch
  actualCount?: number;     // For understaffed (current count)
  requiredCount?: number;   // For understaffed (needed count)
}

// Type of resolution action
export type ResolutionActionType =
  | 'swap'          // Swap two operators' assignments
  | 'assign'        // Assign an unassigned operator to a task
  | 'unassign'      // Remove an assignment
  | 'ignore';       // Mark conflict as accepted (no action)
...
// Complete conflict resolution proposal
export interface ConflictResolution {
  conflictId: string;       // ID of conflict being resolved
  actions: ResolutionAction[];
  // Impact summary
  resolves: string[];       // Conflicts this would resolve
  introduces: string[];     // New conflicts this might introduce (if any)
  confidence: number;       // 0-100, how confident we are this resolves the conflict
}

// Helper to create a swap resolution
export function createSwapResolution(
  conflictId: string,
  day: WeekDay,
  op1Id: string,
  op1Name: string,
  ...
  return {
    ...
    actions: [{
      ...
      operator2Id: op2Id,
      task2Id,
      description: `Swap ${op1Name} (${task1Name}) with ${op2Name} (${task2Name}) on ${day}`,
    }],
    resolves: [`Conflict on ${day}`],
    introduces: [],
    confidence,
  };
}

// Helper to create an assign resolution
export function createAssignResolution(
  conflictId: string,
  day: WeekDay,
  operatorId: string,
  operatorName: string,
  ...
  return {
    ...
    actions: [{
      ...
      operatorId,
      taskId,
      description: `Assign ${operatorName} to ${taskName} on ${day}`,
    }],
    resolves: [`Understaffed ${taskName} on ${day}`],
    introduces: [],
    confidence,
  };
}

// Helper to create an unassign resolution
export function createUnassignResolution(
  conflictId: string,
  day: WeekDay,
  operatorId: string,
  operatorName: string,
  ...
  return {
    ...
    actions: [{
      ...
      operatorId,
      taskId: null,
      description: `Unassign ${operatorName} from ${taskName} on ${day}`,
    }],
    resolves: [`Skill mismatch for ${operatorName} on ${day}`],
    introduces: [`${taskName} may become understaffed on ${day}`],
    confidence,
  };
}

// Helper to create an ignore resolution
export function createIgnoreResolution(
  conflictId: string,
  message: string
): ConflictResolution {
  return {
    ...
    actions: [{
      type: 'ignore',
      day: 'Mon', // Placeholder, not used for ignore
      description: message,
    }],
    resolves: [],
    introduces: [],
